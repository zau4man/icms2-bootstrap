...
function html_pagebar($page, $perpage, $total, $base_uri=false, $query=array(), $page_param_name = 'page'){
	...
	if (($page > 1) || ($page < $pages)) {
		...
		if ($page > 1){
			...
			$html .= ' <li class="page-item disabled"><a class="page-link">&larr; '.LANG_PAGE_PREV.'</a></li> ';
		}
	...
	if ($page > $span+1){
        ...
        $sep = mb_strstr($uri, '?') ? '&' : '?';
        if ($query[$page_param_name] == 1) { unset($query[$page_param_name]); }
        $html .= ' <a href="'. $uri . ($query ? $sep.http_build_query($query) : '') . $anchor . '" class="page-link">'.LANG_PAGE_FIRST.'</a> ';
	}
    ...
    for ($p=$p_start; $p<=$p_end; $p++){
        if ($p != $page) {
            ...
            $html .= '<li class="page-item active"><a href="#" class="page-link">'.$p.'</a>';
        }
    }
	...
	if ($page < $pages - $span){
        ...
        $sep = mb_strstr($uri, '?') ? '&' : '?';
        if ($query[$page_param_name] == 1) { unset($query[$page_param_name]); }
        $html .= '<li class="page-item"><a href="'. $uri . ($query ? $sep.http_build_query($query) : '') . $anchor . '" class="pagebar_page">'.LANG_PAGE_LAST.'</a></li>';
	}
		...
		if ($page < $pages){
			...
			$html .= '<li class="page-item disabled"><a href="#" class="page-link">'.LANG_PAGE_NEXT.' &rarr;</a></li>';
		}
    ...
    $html  .= '<li class="page-item disabled"><a class="page-link" href="#">'.sprintf(LANG_PAGES_SHOWN, $from, $to, $total).'</a></li>';
